  ...
  // console.log(dbnames);
    for (var i =0;i<dbnames.length;i++)
    {
        ...
        if (company)
        {
          ...
          var inserresult=  await insertcallsummerydata(dbname,call_summery_result_data);
...
const update_call_summery_data_v2 = async () => {
    console.log("update_call_summery_data_v2 version is getting executed");
    var dbnamesRaw: any = await getcompanyaccountdbnamesforcron();
    let dbnames = removeDuplicates(dbnamesRaw, "accountid");
    console.log(
    ...
    for (var i = 0; i < dbnames.length; i++) {
        console.log(
            "CallSummery Update , Remaining Companies ",
            dbnames.length - i
        );
        var account = dbnames[i];
        const company = account.type === "company";
        ...
        if (company) {
            const dbname = parseAccountToDatabaseName(companyid);
            let call_summery_result_data2 =
                await calculate_company_callsummery2(
                    companyid,
                    account.industry
                );
            insertcallsummerydata(dbname, {
                data: call_summery_result_data2,
            }).then(()=>{console.log("Call summery inserted for ",companyid)}).catch(e=>{console.log("Call summery not inserted for ",companyid)});   
        } else {
            // console.log("Company id not matched and it is ",companyid)
        }
    }
};
...
const setUpdateCallSummeryJob = async () => {
    // await update_call_summery_data_v2();
    cron.schedule(
        "0 20,50 * * * *",
        async () => {
            console.log("update call_summery_data v2 job started at ",moment().utc());
            // update_call_summery_data();
            console.time("FOR V2");
            await update_call_summery_data_v2();
            console.timeEnd("FOR V2");
            console.log("update call_summery_data v2 job stopped at ",moment().utc());
        },
        {
            scheduled: true,
            timezone: "Etc/UTC",
